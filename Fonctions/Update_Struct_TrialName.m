...
for i = 1 : nargin
    ...
    if isfield(struct_in.Trial(1),'TrialName')
        list_struct = arrayfun(@(i) struct_in.Trial(i).TrialName,1:length(struct_in.Trial),'uni',0);
    else
        champs = fieldnames(struct_in.Trial(1));
        ind = find(arrayfun(@(i) isa(struct_in.Trial(1).(champs{i}),'Signal'),1:length(champs)),1);
        list_struct = arrayfun(@(i) struct_in.Trial(i).(champs{ind}).TrialName,1:length(struct_in.Trial),'uni',0);
    end
...
for i = 1 : nargin
    ...
    struct_out.Removed_Trials = struct_in.Trial(ind_bad);
